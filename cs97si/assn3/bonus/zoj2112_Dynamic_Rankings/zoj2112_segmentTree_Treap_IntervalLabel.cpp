...
struct SegmentTree{
  struct Node{
    ...
    int lo() const { return lch().o;}
    int ro() const { return rch().o;}
    Node(int _L, int _R): o(((_L + _R) | (_L != _R))) , L(_L), R(_R){}
    Node lch() const { return Node(L, m());}
    Node rch() const { return Node(m() + 1, R);}
