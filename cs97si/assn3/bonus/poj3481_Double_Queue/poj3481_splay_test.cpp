...
struct SplayNode{
  ...
  SplayNode(int _key, int _value): key(_key), value(_value), depth(0), father(nil) { ch[0] = ch[1] = nil; }
  SplayNode():key(0), value(0), depth(0), father(this) { ch[0] = ch[1] = this; }
...
struct Splay{
  ...
  SplayNode* splay(SplayNode* o){
    while (o->father != nil){
      SplayNode* fa = o->father;
      if (fa->father == nil) o->zig();
      else if (fa->side() == o->side()){
        fa->zig(); o->zig();
        //o->zig(); o->zig();
      }else{
        fa->zig(); o->zig();
  ...
  SplayNode* findMin(){
    int d = 0;
    while (root->ch[0] != nil) root = root->ch[0], d++;
    root->depth = d;
    return splay(root);
  }
  SplayNode* findMax(){
    int d = 0;
    while (root->ch[1] != nil) root = root->ch[1], d++;
    root->depth = d;
    return splay(root);
  }
...
int main(){
  ...
  cout << "STEP 1" << endl;
  for (int i = 0; i< cnt; i++){
    splay.find(i)->depth, splay.find(1000000 - i)->depth;
  }
  cout << "STEP 2" << endl;
  for (int i = 0; i< cnt; i++){
    splay.find(0)->depth, splay.find(1000000)->depth;
  }
  cout << "STEP 3" << endl;
  for (int i = 0; i< cnt; i++){
    splay.find(i)->depth, splay.find(i)->depth;
  }
  //cout << cnt << endl;
}
