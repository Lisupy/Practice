...
void process(){
  for (int round = 0; round < 13; round++){
    for (int cat = 0; cat < 13; cat++){
      const int score = get_score(dices[round], cat);
      for (int st1 = (1 << 13) - 1; st1 >= 0; st1--){
        if ( (st1 & (1 << cat)) != 0){
          continue;
        }
        if (cal_nbits(st1) != round){
          continue;
        }
        for (int st2 = 63; st2 >=0; st2--){
          ...
          if (s + score + bouns> dp[next_st1][next_st2]){
            dp[next_st1][next_st2] = s + score + bouns;
